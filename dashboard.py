...
class DashboardTab(tk.Frame):
    ...
    def _update_ui(self, kpi_df, line_df, bar_df, pie_df):
        try:
            ...
            # Update line chart
            self.ax_line.clear()
            self.ax_line.set_title("Daily Trends (Last 7 Days)", fontsize=10, fontweight='bold', color=primary_color)
            if not line_df.empty:
                # Use only M-D format for better fit
            ...
            # Update bar chart
            self.ax_bar.clear()
            self.ax_bar.set_title("Monthly Performance (Last 6 Months)", fontsize=10, fontweight='bold', color=primary_color)
            if not bar_df.empty:
            ...
            # Update pie chart
            self.ax_pie.clear()
            self.ax_pie.set_title("Status Distribution", fontsize=10, fontweight='bold', color=primary_color)
            if not pie_df.empty:
                # Colorful pie chart based on app colors
                ...
                self.ax_pie.pie(pie_df['Count'], labels=pie_df['OrderStatus'], autopct='%1.1f%%', startangle=90, colors=colors, textprops={'fontsize': 8})
